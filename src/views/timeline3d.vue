...
const init = () => {
  ...
  for (let i = 0; i < tableList.value.length; i++) {
    ...
    cover.className = "element-cover";
    let img: any = document.createElement("img");
    img.src = tableList.value[i].avatar;
    ...
    nameSpan.textContent = tableList.value[i].name;
    name.appendChild(nameSpan);
    console.log(backgroundIndex)
    name.style.backgroundImage = `url(${backgroundArr[backgroundIndex]})`;
  ...
  if (document.getElementById("container")) {
    console.log(renderer.value.domElement);
    //@ts-ignore
...
const transform = (e, targets, duration) => {
  TWEEN.removeAll();
  for (let i = 0; i < objects.value.length; i++) {
...
<template>
  <div class="timeline3d" ref="timeline3dRef">
    <div class="menu">
      <!-- <button id="table" @click="transform($event, table, 2000)">TABLE</button> -->
      <button id="sphere" @click="transform($event, sphere, 2000)">球型</button>
      <button id="helix" @click="transform($event, helix, 2000)">螺旋</button>
      <button id="grid" @click="transform($event, grid, 2000)">网格</button>
    </div>
